...
needed, and **nothing pre-checked**. Take the clipboard out, **tick** the blocks you don't want placed, and
...
- **Skip any block type** — tick the entries you want the cannon to leave alone; ticking is per *item*, so it
  covers every block and block entity variant that needs that item.
...
**Tick** the entries for the blocks you want the cannon *not* to place — e.g. decorative fills, glass,
torches, or anything you'd rather place by hand.

> Ticking skips *every* placement that needs that item, including variants and block entities that drop it.

### 4. Reinsert and print

Put the ticked clipboard back into the **list-printer slot**. The cannon now treats it as the active
skip list: it will **not place** any checked block, and the on-screen placed/needed counts update to match.
